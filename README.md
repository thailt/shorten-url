...
- **Create Short URLs**: Generate short aliases for long URLs with optional expiration dates
...
- **Purpose**: Generates time-ordered IDs for distributed systems
- **Use Case**: Current default key generation strategy for scalable deployments
- **Benefits**:
  - Very low collision risk in distributed deployments
...
- **Purpose**: Intended durable storage for alias mappings
- **Use Case**: Source of truth after write-behind batches are flushed successfully
- **Benefits**:
  - Transaction support
  - Relational data integrity
  - Connection pooling with HikariCP

**Note:** create responses are returned before batched persistence completes, so durability is eventual rather than immediate at response time.
...
1. **Multi-layer Caching**:
   - L1: In-memory LRU cache (1000 entries, fastest)
   - L2: Redis cache (10-day TTL, sub-millisecond)
   - L3: MySQL database (intended durable store, slower)
